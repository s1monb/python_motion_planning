...
class DStar3D(GraphSearcher3D):
    ...
    Examples:
        ...
        >>> planner = pmp.DStar((5, 5), (45, 25), pmp.Grid3D(51, 31))
    ...
    def OnPress(self, event) -> None:
        ...
        """
        x, y, z = int(event.xdata), int(event.ydata), int(event.zdata)
        if x < 0 or x > self.env.x_range - 1 or y < 0 or y > self.env.y_range - 1 or z < 0 or z > self.env.z_range - 1:
    ...
    def insert(self, node: DNode3D, h_new: float) -> None:
        ...
        Parameters:
            node (DNode3D): the node to insert
    ...
    def delete(self, node: DNode3D) -> None:
        ...
        Parameters:
            node (DNode3D): the node to delete
        """
        if node.t == 'OPEN':
            node.t = 'CLOSED'
    ...
    def modify(self, node: DNode3D, node_parent: DNode3D) -> None:
        ...
        Parameters:
            node (DNode3D): the node to modify
            node_parent (DNode3D): the parent node of `node`
        """
        if node.t == 'CLOSED':
